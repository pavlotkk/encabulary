...
class AutoCompleteAPI(MethodView):
    def get(self, word, id_word_type=None):
        ...
        translation_items = linguee_dictionary.get_items(word)
        if not translation_items:
            reverso_dictionary = ReversoDictionary()
            translation_items = reverso_dictionary.get_items(word)

        translations = [item.translations for item in translation_items if
